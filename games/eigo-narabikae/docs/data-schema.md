...
## 9. 出題画面（quiz screen）の仕様

- 各questionは日本語訳 `jp` フィールドを持つ（必須、`qa/validate.js` が検証）。出題画面では単語ブロックのエリアの**上**に表示する。単元名などの「どの文法項目か」を示すタグは出題中は表示しない（ヒントになってしまうため）。
- `answer` の最後のトークンが `.` または `?` の場合、それは単語バンクには出さず、スロットの末尾に最初から埋めた状態（`.slot.locked`、タップ不可）で表示する。文中の `,`（接続詞・仮定法などのカンマ区切り）は対象外で、通常どおりタップして並べる単語タイル。
- 大文字化は次の2つを組み合わせて行う：
  1. `PROPER_NOUNS` / `alwaysCapitalize` による固有名詞・"I"・"I'll"/"I'm" などの常時大文字化（位置に関係なく適用）。
  2. 組み立て中のスロットの**1番目（左端）**に置かれた単語は、それが何であっても（"the" や "he" のような固有名詞でない語でも）先頭が大文字で表示される（`capitalizeFirst()`）。これは正解表示時の `formatDisplay()` と同じ規則を、組み立て中のリアルタイム表示にも適用したもの。
